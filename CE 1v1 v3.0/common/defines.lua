...
country = {
	...
	COLONIAL_RANK = 2, -- Minimum rank a nation must have to send colonists
	COLONY_TO_STATE_PRESTIGE_GAIN = 4, -- Prestige gain when turning colony to state
	COLONIAL_LIFERATING = 80,
	...
	BASE_MONTHLY_DIPLOPOINTS = 20, -- Base value gain for diplomatic actions each month
	...
	NEWSPAPER_PRINTING_FREQUENCY = 3500000, -- days frequency when the news attempt to be printed (may not print if not enough facts collected, and retry after another X days)
	NEWSPAPER_TIMEOUT_PERIOD = 8900000, -- the max period for news that may be printed. The probability is decreased with time.
	NEWSPAPER_MAX_TENSION = 7500000, -- when tension of printing gets this high, the newspaper will attempt to be printed immediately.
	...
	MOBILIZATION_SPEED_BASE = 1, -- Base speed for raising troops
